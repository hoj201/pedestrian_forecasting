...
\begin{thm} \label{thm:FP}
	...
	Moreover, assume the initial condition $\rho^+(0,x) = \rho^-(0,x) = P(x \mid c_k , \mu_0 )$.
...
and the computation of a few conditional probabilities at time $t=0$ (these are given in Appendix \ref{app:prob}).
...
\subsection{Probability computations} \label{app:probs}
...
By assumption, $x_0$ is independent of velocity information.
Thus $P(x_0 \mid \eta_0 ,\mu_0, c_k, s ) = P(x_0 \mid \mu_0 , c_k)$.
...
We also need to compute the coefficient $P(c_k,s \mid \mu_0, \eta_0 )$.
By Bayes' theorem we find
\begin{align*}
	P(c_k,s \mid \mu_0, \eta_0 ) = P(s \mid \mu_0, \eta_0, c_k ) P(c_k \mid \mu_0, \eta_0 )
\end{align*}
Again, by Bayes' theorem
\begin{align*}
	P(s \mid c , \eta_0, \mu_0 ) = \int P(s \mid c , \eta_0, \mu_0 , x_0, v_0 ) P( x_0, v_0 \mid \mu_0, \eta_0 ) dx_0 dv_0.
\end{align*}
The speed $s$ is independent of the measurements $\eta_0$ and $\mu_0$ when the exact position and velocity are known.
Thus $P(s \mid c,\eta_0, \mu_0, x_0, v_0) = P(s \mid c , x_0, v_0 )$.
Moreover, if the agent is in class $c$ at position $x_0$  with velocity $v_0$, then $s$ is the unique scalar such that $s X_c(x_0) = v_0$.
Thus $P(s \mid c, x_0, v_0)$ is a Dirac-delta like distribution on the subspace described by $s X_c(x_0) = v_0$.
The integral then becomes
\begin{align*}
&P(s \mid c , \eta_0, \mu_0 ) = \int_{sX_c(x_0) = v_0} P( x_0, v_0 \mid \mu_0, \eta_0 ) dx_0 dv_0 \\
&= \int_{sX_c(x_0) = v_0 } P(x_0 \mid \mu_0 ) P( v_0 \mid \eta_0 ) dx_0 dv_0 \\
&= \frac{1}{ 2 \pi \sigma_x \sigma_v } \int \exp \left( \frac{- \| x_0 - \mu_0 \|^2 }{2 \sigma_x^2 } + \frac{ - \| s X_c(x_0) - \eta_0 \| }{ 2 \sigma_v^2 } \right) dx_0
\end{align*}

Finally, we must compute $P(c_k \mid \eta_0, \mu_0)$.
By Bayes' theorem
\begin{equation}
\begin{split}
	& P(c_k \mid \eta_0 , \mu_0 ) = \\
	&\quad \int P(c_k \mid v_0 , x_0 , \mu_0 , \eta_0 ) P( v_0, x_0 \mid \mu_0, \eta_0 ) dx_0 dv_0 \label{eq: c given measurements}
\end{split}
\end{equation}
where $v_0$ is a dummy variable for the ``true'' velocity.
Assuming independence of measurements in time we find
\begin{align*}
	P(v_0,x_0 \mid \mu_0 , \eta_0 ) &= P( x_0 \mid \mu_0 , \eta_0 ) P( v_0 \mid \mu_0 , \eta_0 ) \\
	&=P( x_0 \mid \mu_0 ) P( v_0 \mid \eta_0 ) \\
	&= \frac{1}{ 2\pi \sigma_m \sigma_v } \exp \left( \frac{- \|x_0 - \mu_0 \|^2 }{2\sigma_m^2} \right) \\
	&\quad \cdot \exp \left( \frac{- \|v_0 - \eta_0 \|^2 }{ 2\sigma_v^2 } \right)
\end{align*}
Moreover, $c_k$ is independent of the measurements given we know the true position and velocity.
That is $P(c_k \mid v_0, x_0, \mu_0, \eta_0 ) = P(c \mid x_0, v_0 )$.
The latter can be computed precisely
\begin{align*}
	P( c_k \mid x_0 , v_0 ) =
	\begin{cases}
		1 \text{ if } X_{c_k}(x_0) = v_0 \\
		0 \text{ else }
	\end{cases}
\end{align*}
Substitution into \eqref{eq: c given measurements} reveals
\begin{equation}
\begin{split}
	& P(c_k \mid \eta_0 , \mu_0 ) = \\
	&\quad \frac{1}{4\pi^2 \sigma_x^2 \sigma_v^2 } \int \exp \left( \frac{ - \| x_0 - \mu_0 \| }{ 2 \sigma_m^2} + \frac{ - \| \eta_0 - X_c( x_0 ) \| }{ 2 \sigma_v^2} \right) dx_0 
\end{split} \label{eq: c given measurements}
\end{equation}

where we have assumed that $X_{c_i}(x_0) \neq X_{c_j}(x_0)$ almost surely in $x_0$ (this is certainly the case for vector-fields we'd learn from in practice).
...
In the case of a linear predictor, all velocities are equally valid and space is homogenous.
Thus $c_0$ adds no information given measurement $\mu_0$ and $\eta_0$ and the above line becomes
\begin{align*}
	&= \int P(x_0 = x_T - Tv_0 \mid \mu_0 ) P( v_0 \mid \eta_0 ) dv_0 \\
	&= \frac{1}{ 4\pi^2 \sigma_x^2 \sigma_v^2 } \int \exp \left( \frac{ - \| x_T - Tv_0 - \mu_0 \| }{ 2 \sigma_x^2} + \frac{ - \| v_0 - \eta_0 \|^2 }{ 2 \sigma_v^2 } \right) dv_0 
\end{align*}
Moreover, $P(c_0 \mid \eta_0 , \mu_0 ) = 1 - \sum_{k=1}^n P(c_k \mid \eta_0, \mu_0)$
where $P(c_k \mid \eta_0, \mu_0 )$ for $k=1,\dots,n$ is given by \eqref{eq: c given measurements}
