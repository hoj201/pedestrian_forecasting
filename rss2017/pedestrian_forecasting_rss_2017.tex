...
The present article is concerned with a probabilistic forecast for a single agent, while ignoring interactions with other agents and other related tasks, such as detection of agents and environmental features.
More specifically, the present article concerns the construction of useful motion models for agents.
...
In particular, such models were only found to be reasonable for pedestrian prediction over times below $2$ seconds, especially when the pedestrian turns.
...
They opted to use a sample based method: a Rao-Blackwellized particle filter\cite{Doucet2000}.
...
Along similar lines, \citet{Walker2014} has constructed a completely unsupervised approach towards forecasting, trained on unlabeled video data.
As before, the motion model is that of a discrete time Markov process.  However, the agents are not manually specified, but are learned from a database of videos by detecting which sort of patches are likely to move, and how.  The resulting predictions outperformed \cite{Kitani2012} when comparing the most likely path with the ground truth in mean Hausdorff distance.
As in all methods mentioned thus far, computational speed was not a concern of \citet{Walker2014}, and no compute times were given.
Since the motion model is a discrete time Markov process over a continuous space, the same limits that apply to \cite{Karasev2016,Ballan2016} in terms of speed hold for \cite{Walker2014} as well.
In particular, such motion models force the use of sample-based representations of the posteriors in question with probabilistic descriptions of error, making high accuracy computation very expensive.
...
\todo[inline]{hoj:I deleted a bunch of details.  But as I was writing the section on fast computation, I found I was adding them back in.  Perhaps the wording can be economized though.}
