...
\begin{align*}
	...
	&\quad + \Pr( x_t, lin \mid \hat{x}_0, \hat{v}_0 ).
\end{align*}
The final term, $\Pr( x_t, lin  \mid \hat{x}_0, \hat{v}_0)$, turns out to be a closed form expression involving error-functions.\todo{hoj:Must I provide this?  It's a bit long.}
...
where $v_0(s,k,x_0) := s\, X_k(x_0)$. As the density $\Pr(\hat{x}_0, x_0, \hat{v}_0, v_0(s,k,x_0), k, s) $ is Riemann integrable in $x_0$,
 we may approximate it as a sum of weighted Dirac delta's supported on a regular grid with spacing $\Delta x$.\todo{Perhaps prove this claim in the appendix.}
In other words
\begin{align}
	\begin{split}
