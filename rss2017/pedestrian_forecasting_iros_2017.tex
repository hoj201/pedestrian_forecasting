...
In cases where there are many locally optimal solutions between a start and end goal, IOC type methods can yield non-robust and imprecise behavior.
...
\begin{itemize}
	\item We provide an accurate motion model for pedestrian forecasting.
	\item We provide expedient methods for computing approximations of relevant posteriors generated by our motion model.
	\item We prove hard error bounds on the proposed approximations.
\end{itemize}
...
\begin{itemize}
	...
	\item Finally, we demonstrate the model by training and testing it on the Stanford drone dataset \cite{Robicquet2016} (\S \ref{sec:implementation}).
\end{itemize}
...
\begin{align*}
	\Pr( \check{x}_t \mid x_0, v_0, lin) = \delta( \check{x}_t - x_0 - t v_0 ) d\check{x}_t.
\end{align*}
where $\delta( \cdot )$ denotes the multivariate Dirac-delta distribution.
...
\end{align}
The final term $\Pr( x_t, lin \mid \hat{x}_0, \hat{v}_0 )$ is expressible in closed form when the posteriors $\Pr( x_0 \mid lin)$ and $\Pr( v_0 \mid lin,x_0)$ are simple expressions such as Gaussians or uniform distributions. In either case, numerical computation of $\Pr(x_t, lin \mid \hat{x}_0, \hat{v}_0)$ poses a negligible burden in terms of numerical computation.
...
\end{align*}
Substitution of \eqref{eq:x_check | ksx} and \eqref{eq:v | ksx} yields
\begin{align*}
...
At this point, we can state an algorithm for computing an $\mathcal{O}( \Delta x)$ approximation of $\rho_t(x_t)$:
\begin{algorithm}
	Compute the constants $c_{k,s,\alpha}$ from \eqref{eq:constants} \;
	Compute $\Phi_{k,1}^t(x_0^{\alpha})$ for each $k$ and each point $x_0^{\alpha}$ in the grid over a time-interval $[-T,T]$.\;
	Obtain $\Phi_{k,s}^t( x_0^\alpha)$ from $\Phi_{k,1}^t(x_0^{\alpha})$ (Theorem \ref{thm:symmetry}).\;
	Obtain an $\mathcal{O}( | \Delta x | )$ approximation of $\Pr(x_t ,k,s,\hat{x}_0 , \hat{v}_0)$ (Corollary \ref{cor:error}).\;
	Use \eqref{eq:pgm} and \eqref{eq:decomposition} to compute $\Pr( x_t, \hat{x}_0, \hat{v}_0)$.\;
	Compute $\mu = \int \Pr( x_t, \hat{x}_0, \hat{v}_0) dx_t$\;
	Compute the division $\rho_t( x_t) = \Pr(x_t , \hat{x}_0, \hat{v}_0 ) / \mu$.\;
	\caption{Our algorithm for computing $\rho_t$.}
\end{algorithm}

Steps 1 and 2 are where the bulk of the work occurs.
...
\begin{thm}
	Assuming we have an accurate scheme for integrating ODEs in a computation time proportional to the integration time-interval\footnote{For example, 4th order explicit Runge-Kutta}.  Algorithm 1 yields an $\mathcal{O}( \Delta x)$ accurate approximation of $\rho_t$ in $\mathcal{O}( T n | \Gamma | / N )$ time given $N$ processing units and $n$ vector fields.
\end{thm}
The proof is a matter of some simple book-keeping.
