...
\author{We can put our names in here, also lets make sure we are actually using the IROS template...}
...
The resulting algorithm is embarrassingly parallelizeable, and trained on historical trajectory information within a fiscene.
...
\todo[inline]{Seriously lets make this real.}
\end{abstract}
...
For example, \citet{Kooji2014} uses a motion model of this variety, parametrized by latent variables, such as the pedestrian's awareness of nearby vehicles, to predict when and how a pedestrian may cross a street without leveraging any environmental context.
...
For example, \citet{Kitani2012} extracted a Markov Decision Process (MDP) evolving on a finite 2D lattice by training on a small collection of features and trajectories.
...
The approach presented by \citet{Kitani2012} has been generalized in a variety of ways.
For example, time-dependent information, such as traffic signals, are incorporated by \citet{Karasev2016}, by relaxing the Markov property and considering a switched Markov process.
Other generalizations by \citet{Karasev2016} include replacing the finite-state space with a continuous one, and using a Markov jump process in the motion model.
...
Similar to \citet{Karasev2016}, the motion model was a Markov jump process and the relevant posteriors were approximated using sample based techniques.
...
We assume that given the true position, $x_0 \in \mathbb{R}^2$, that the measurement $\hat{x}_0$ is independent of all other variables and the posterior $\Pr( \hat{x}_0 \mid x_0)$
