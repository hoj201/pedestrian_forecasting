...
def rho_true(subj, T, test_set, bbox_ls, test_scene):
...
if __name__ == "__main__":
    ...
    import types
    def f(name, ind, i, test_set, test_scene):
        scene = test_scene
        ...
        ppr_lin = np.array([])
        ttr_lin = np.array([])
        reference = mpimg.imread(folders[sn] + "reference.jpg")
        for ((x_arr_ours, w_arr_ours), (x_arr_lin, w_arr_lin)) in ours:
            if n == 0:
                n += 1
                continue
            if n % 5 == 0:
                plt.scatter(x_arr_ours[0], x_arr_ours[1], c=w_arr_ours, cmap="viridis", edgecolors="none")
                ...
                rt = lambda x: rho_true(i, int(t_final/float(N_steps) * n), test_set, x, test_scene)
                ...
                plt.savefig("images/precision_recall/DOOT_DOOT/pr_agent{}_T{}.png".format( i, int(t_final/float(N_steps) * n)))
                #plt.show()
                #plt.close('all')
            n += 1
    ...
    generate_distributions.set_scene(-1, custom_scene=data[0][0])
    f(name, 0, 1, test_set, data[0][0])    
